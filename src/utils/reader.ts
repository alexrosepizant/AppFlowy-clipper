...
export class Reader {
	...
	private static async loadSettings(): Promise<void> {
		const savedSettings = await browser.storage.sync
			.get("reader_settings")
	...
	private static injectSettingsBar(doc: Document) {
		...
		for (const { action, icon } of clipActions) {
			...
			item.addEventListener("click", async () => {
				if (action === "copyToClipboard") {
					...
					setTimeout(() => {
						itemLabel.textContent = originalText;
					}, 2000);
		...
		doc.addEventListener("click", (e) => {
			if (
				!clipButton.contains(e.target as Node) &&
				!clipDropdown.contains(e.target as Node)
			) {
				clipDropdown.classList.remove("is-open");
			}
		});
		...
		increaseWidthBtn.appendChild(
			this.createSVG({
				...
				viewBox: "0 0 24 24",
				className:
					"lucide lucide-move-horizontal-icon lucide-move-horizontal",
		...
		fontWrapper.appendChild(
			this.createSVG({
				...
				viewBox: "0 0 24 24",
				strokeWidth: "1.75",
				paths: ["M4 7V4h16v3", "M9 20h6", "M12 4v16"],
			})
		);
		...
		for (const font of [...this.settings.fonts].sort((a, b) =>
			a.localeCompare(b)
		)) {
			const option = doc.createElement("option");
			option.value = font;
		...
		settingsBar.addEventListener("click", (e) => {
			...
			switch (action) {
				case "decrease-font":
					this.updateFontSize(
						doc,
						parseInt(style.getPropertyValue("--font-text-size")) - 1
					);
					break;
				case "increase-font":
					this.updateFontSize(
				...
				case "decrease-width":
					this.updateWidth(
						doc,
						parseInt(style.getPropertyValue("--line-width")) - 1
					);
					break;
				case "increase-width":
					this.updateWidth(
				...
				case "decrease-line-height":
					this.updateLineHeight(
						doc,
						parseFloat(
							style.getPropertyValue("--line-height-normal")
						) - 0.1
					);
					break;
		...
		themeModeSelect.addEventListener("change", () => {
			this.updateThemeMode(
				doc,
				themeModeSelect.value as "auto" | "light" | "dark"
			);
		});
	...
	private static updateThemeMode(
		doc: Document,
	...
	private static generateOutline(doc: Document, title?: string) {
		...
		const outline = doc.querySelector(
		...
		if (title && titleHeading) {
			...
			titleItem.className =
				"obsidian-reader-outline-item obsidian-reader-outline-h1";
		...
		const lastHeadingAtLevel: {
			[key: number]: { element: Element; depth: number };
		} = {};
		...
		headings.forEach((heading) => {
			...
			for (let i = level - 1; i >= 2; i--) {
				...
				if (lastHeading) {
					const lastRect =
						lastHeading.element.getBoundingClientRect();
			...
			if (!parentFound && lastHeadingAtLevel[level]) {
				const lastRect =
					lastHeadingAtLevel[level].element.getBoundingClientRect();
				if (lastRect.top < currentRect.top) {
	...
	private static initializeFootnotes(doc: Document) {
		...
		const footnoteItems = doc.querySelectorAll(
			'#footnotes ol > li[id^="fn:"]'
		);
		...
		this.footnoteClickHandler = (e: Event) => {
			...
			const backrefLink = target.closest(
				"a.footnote-backref"
			...
			if (footnoteLink) {
				...
				if (footnote) {
					...
					const returnLink = content.querySelector(
						'a[title="return to article"]'
					);
					returnLink?.remove();

					// Show popover
					popover.textContent = "";
					const clonedContent = content.cloneNode(
						true
					) as HTMLElement;
		...
		this.footnoteScrollHandler = () => {
			if (this.activeFootnoteLink && this.activePopover) {
				this.positionPopover(
					this.activePopover,
					this.activeFootnoteLink
				);
			}
		};
		this.footnoteResizeHandler = this.footnoteScrollHandler;
		doc.addEventListener("scroll", this.footnoteScrollHandler, {
			passive: true,
		});
		window.addEventListener("resize", this.footnoteResizeHandler);
	}

	private static showFootnotePopover(
		popover: HTMLElement,
	...
	private static positionPopover(
		popover: HTMLElement,
		link: HTMLAnchorElement
	) {
		...
		const spaceBelow =
			viewportHeight - linkRect.bottom - ARROW_HEIGHT - VIEWPORT_PADDING;
		...
		if (currentTop < VIEWPORT_PADDING) {
			...
			if (bottomOverflow > 0) {
				popover.style.maxHeight = `${
					popoverRect.height - bottomOverflow
				}px`;
				popover.style.overflowY = "auto";
			}
		}
			...
			if (
				...
				while (id--) {
					try {
						...
						console.log(
							"Reader",
							"Error clearing timeout/interval:",
							e
						);
					}
				}
			...
			button.addEventListener("click", async () => {
				try {
					...
					if (success) {
						...
						console.log(
							"Reader",
							"Error copying code: clipboard operation failed"
						);
					}
				} catch (err) {
	...
	private static linkifyTextUrls(doc: Document): void {
		...
		const walker = doc.createTreeWalker(article, NodeFilter.SHOW_TEXT, {
			acceptNode(node) {
				...
				return urlPattern.test(node.textContent || "")
	...
	private static initializeComments(doc: Document) {
		...
		commentsEl
			.querySelectorAll<HTMLElement>(".comment-author")
			.forEach((authorEl) => {
				const username =
					authorEl.querySelector("strong")?.textContent?.trim() || "";
				if (!username) return;
				authorEl.style.backgroundColor = `color-mix(in srgb, var(${this.usernameToColor(
					username
				)}) 20%, transparent)`;
			});
		...
		commentsEl
			...
			.forEach((comment) => {
				...
				btn.setAttribute(
					"aria-label",
					getMessage("readerCollapseComment")
				);
				...
				const isTopLevel = comment.parentElement?.matches?.(
					".comments > blockquote"
				);
		...
		commentsEl.addEventListener("mouseover", (e) => {
			...
			if (
				newAncestors.length === highlightedAncestors.length &&
				...
				if (parent.parentElement?.matches?.(".comments")) {
					...
					while (sibling) {
						if (sibling.tagName === "BLOCKQUOTE") {
							(sibling as HTMLElement).classList.add(
								"child-hovered"
							);
	...
	private static initializeLightbox(doc: Document) {
		...
		closeButton.setAttribute(
			"aria-label",
			getMessage("readerCloseImageViewer")
		);
		...
		if (article) {
			...
			const linkedImages = Array.from(
				article.querySelectorAll("a:not(figure a) img")
			).filter((img) => {
	...
	private static showLightbox(index: number) {
		...
		const container = this.lightbox.querySelector(
			".lightbox-image-container"
		);
		const captionContainer =
			this.lightbox.querySelector(".lightbox-caption");
		...
		if (container && captionContainer) {
			...
			const figure = (this.images[index] as any)
				.figureElement as HTMLElement;
			if (figure) {
				const figcaption = figure.querySelector("figcaption");
				if (figcaption) {
					const clonedCaption = figcaption.cloneNode(
						true
					) as HTMLElement;
	...
	static async apply(doc: Document) {
		...
		try {
			...
			const existingViewport = head.querySelector(
				'meta[name="viewport"]'
			);
			if (existingViewport) {
			...
			this.populateArticle(doc, main, article, {
				content,
				title,
				author,
				published,
		...
		article.addEventListener("click", (e: Event) => {
			...
			if (this.isReaderPage && this.onNavigate) {
				...
				window.location.href = browser.runtime.getURL(
					"reader.html?url=" + encodeURIComponent(href)
				);
			} else {
				browser.runtime.sendMessage({
	...
	private static populateArticle(
				...
				if (item.type === "author") {
					...
					this.getHighlightCountForDomain(domain).then((count) => {
						if (count > 0) {
							...
							countLink.textContent = `${count} highlight${
								count === 1 ? "" : "s"
							}`;
		...
		if (content.content) {
			...
			const contentDoc = contentParser.parseFromString(
				content.content,
				"text/html"
			);
		...
		if (footer) {
			const footerItems = [
				"Obsidian Reader",
				content.wordCount
					? new Intl.NumberFormat().format(content.wordCount) +
					  " words"
					: "",
				content.parseTime
					? "parsed in " +
					  new Intl.NumberFormat().format(content.parseTime) +
					  " ms"
					: "",
			].filter(Boolean);
	...
	// Shared between apply() and updateReaderContent().
	private static async initializeContentFeatures(
		doc: Document,
		title?: string
	): Promise<void> {
	...
	private static async getHighlightCountForDomain(
		...
		const allHighlights = (result.highlights || {}) as Record<
			string,
			{ highlights: any[]; url: string }
		>;
		let count = 0;
		for (const stored of Object.values(allHighlights)) {
			try {
				const hostname = new URL(stored.url).hostname.replace(
					/^www\./,
					""
				);
	...
	static async toggleReaderPageIframe(doc: Document): Promise<void> {
		...
		const { clipperIframeWidth, clipperIframeHeight } =
		...
		iframe.src = browser.runtime.getURL(
			"side-panel.html?context=iframe&readerUrl=" +
